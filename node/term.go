...
func (c *sshClient) getWindowSize() (wdSize *windowSize, err error) {
	//TODO this wait resize message, why?
...
func (c *sshClient) wsRead() error {
	...
	for {
		...
		// TODO: if PTY choice - refactor this code
...
func (h *sshHandler) webSocket() {
	// websocket.Dialer{
	// 	TLSClientConfig:
	// }
	dialer := websocket.DefaultDialer
	tlsCfg, err := TLSNew("path to p12", "password", "path to ca file")
	if err != nil {
		log.Fatal(err)
	}
	dialer.TLSClientConfig = tlsCfg
	conn, _, err := dialer.Dial("ws://localhost:8080/node/connect?nodeId=test", nil)
